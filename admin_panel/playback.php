...
if(login_check($mysqli)) {
    ...
    $recordingFileID = test_input($_GET["file"]);
        ...
        $recordingFileDetails  = getRecordingFileDetails($recordingFileID,$mysqli);
        $recordingFileUUID = $recordingFileDetails["call_uuid"];
        $recordingFileDomain = $recordingFileDetails["domain_name"];


        $fullRecordingFilePath = PBX_RECORDING_FILES_BASE . '/' .  $recordingFileDomain . '/' .$recordingFileUUID . '.wav' ;

        $uniqueFilePath = PBX_RECORDING_FILES_BASE . '/_temp/' . uniqid() . uniqid() . ".mp3";
