...
function serveWebsocketStreamingConfiguration($domain, $request, $domainDetails, $customerDetails, $caller, $mysqli) {
    ...
    $recordingDestinationShort = '/opt/ctpbx/recordings/' . $domain . '/' . $channel_uuid . '.wav';
    error_log("====== WEBSOCKET STREAMING: Recording path: " . $recordingDestinationShort);
    ...
    // Set WebSocket stream sample rate to 16kHz for better quality
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'set');
    $defaultContextConfiguration->writeAttribute('data', 'STREAM_SAMPLE_RATE=16000');
    $defaultContextConfiguration->endElement(); // action

    // === CALL RECORDING (Enhanced for WebSocket audio capture) ===
    // Enable stereo recording to capture both legs (caller + WebSocket)
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'set');
    $defaultContextConfiguration->writeAttribute('data', 'RECORD_STEREO=true');
    $defaultContextConfiguration->endElement(); // action

    // Force recording to capture all audio including mod_audio_stream playback
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'set');
    $defaultContextConfiguration->writeAttribute('data', 'record_waste_resources=true');
    $defaultContextConfiguration->endElement(); // action

    // Enable recording of conference audio
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'set');
    $defaultContextConfiguration->writeAttribute('data', 'RECORD_BRIDGE_PEER=true');
    $defaultContextConfiguration->endElement(); // action
    ...
    // Note: We'll start recording after WebSocket connection to capture mixed audio
    // This ensures CDR gets the complete conversation (caller + WebSocket)
    // === END CALL RECORDING SETUP ===
    ...
    // Connect to WebSocket for audio streaming at 16kHz
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'set');
    $defaultContextConfiguration->writeAttribute('data', 'api_result=${uuid_audio_stream(${uuid} start ' . $websocket_url . ' mono 16k)}');
    $defaultContextConfiguration->endElement(); // action
    ...
    // WebSocket recording removed - using ONLY conference recording
    ...
    // Log conference bridge start
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'log');
    $defaultContextConfiguration->writeAttribute('data', 'INFO Bridging call to WebSocket conference - UUID: ${uuid}');
    $defaultContextConfiguration->endElement(); // action

    // Conference name for WebSocket streaming
    $conference_name = 'websocket-stream-' . $channel_uuid;

    // PHP error_log to track recording path and conference details
    error_log("====== CONFERENCE RECORDING DEBUG ======");
    error_log("Recording path: " . $recordingDestinationShort);
    error_log("Conference name: " . $conference_name);
    error_log("Channel UUID: " . $channel_uuid);
    error_log("Domain: " . $domain);
    error_log("=========================================");

    // Try different auto-record syntax - some versions use 'record' instead of 'auto-record'
    $conference_command = $conference_name . '@default+flags{audio-always|record=' . $recordingDestinationShort . '}';
    error_log("====== CONFERENCE COMMAND: " . $conference_command);
    
    // Also try manual conference record as backup
    error_log("====== BACKUP MANUAL RECORD COMMAND: conference " . $conference_name . " record " . $recordingDestinationShort);

    // Bridge the call into conference with auto-record parameter
    // This starts recording automatically when the conference begins
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'conference');
    $defaultContextConfiguration->writeAttribute('data', $conference_command);
    $defaultContextConfiguration->endElement(); // action

    // Log conference command in FreeSWITCH
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'log');
    $defaultContextConfiguration->writeAttribute('data', 'INFO CONFERENCE COMMAND: ' . $conference_command);
    $defaultContextConfiguration->endElement(); // action

    // Manual conference record as backup (after joining)
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'conference');
    $defaultContextConfiguration->writeAttribute('data', $conference_name . ' record ' . $recordingDestinationShort);
    $defaultContextConfiguration->endElement(); // action

    // Log manual record attempt
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'log');
    $defaultContextConfiguration->writeAttribute('data', 'INFO MANUAL CONFERENCE RECORD: ' . $conference_name . ' record ' . $recordingDestinationShort);
    $defaultContextConfiguration->endElement(); // action

    // Create a virtual leg to join the conference (needed for recording to work properly)
    // Some conference recording requires at least 2 members
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'set');
    $defaultContextConfiguration->writeAttribute('data', 'originate_result=${originate(null/null &conference(' . $conference_name . '@default+flags{mute|deaf}))}');
    $defaultContextConfiguration->endElement(); // action

    // Log virtual leg creation
    $defaultContextConfiguration->startElement('action');
    $defaultContextConfiguration->writeAttribute('application', 'log');
    $defaultContextConfiguration->writeAttribute('data', 'INFO VIRTUAL LEG CREATED: ${originate_result} - Conference should now have 2 members for recording');
    $defaultContextConfiguration->endElement(); // action
